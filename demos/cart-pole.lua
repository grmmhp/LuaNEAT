...
local initial_angle = -math.pi/2 + math.rad(2)
...
local function get_neural_nets(pool)
  local networks={}
  for s=1,#pool.species do
    for g=1,#pool.species[s].genomes do
      table.insert(networks, pool.species[s].genomes[g]:buildNeuralNetwork())
    end
  end
...
-- neat
local pool = neat.newPool(100, 2, 2)--4, 2)
pool:initialize()
...
function cartpole.update(dt)
  ...
  or time_surv > max_time then -- we cut the simulation if it surpasses 2 minutes
    net:setFitness(time_surv)
    time_surv = 0
    reset_sim()
...
function cartpole.draw()
  ...
  love.graphics.print(
    ...
    "\nGenome ID: ".. networks[evaluating].genome.id ..
    "\nFitness: ".. time_surv ..
